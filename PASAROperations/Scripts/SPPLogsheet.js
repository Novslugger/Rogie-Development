    ...
    $.ajax({
        ...
        success: function (data) {
            $.each(data, function (index, value) {
                ...
                $('#paramid_bin_actual').text();
                //check_actual((value.KPI_CuLossInSlag_val).toFixed(2), "paramid_bin_actual");
...
function load_target_numeric(date_) {
    $.ajax({
        ...
        success: function (data) {
            $.each(data, function (index, value) {
                $('#paramid_fsffeed_target').text((value.FSFFeed_Target_val).toFixed(2));
                $('#paramid_kmsd_target').text((value.KUMERASDFeed_Target_val).toFixed(2));
                $('#paramid_optime_target').text((value.OperatingHours_Target_val).toFixed(2));
                $('#paramid_mtgrd_target').text((value.KPI_MatteGrade_Target_val).toFixed(2) + '%');
                $('#paramid_mtlvl_target').text((value.KPI_MatteLevel_Target_val).toFixed(2));
                $('#paramid_mttmp_target').text((value.KPI_MatteTemp_Target_val).toFixed(2));
                $('#paramid_cuinfssl_target').text((value.KPI_CUinFSSL_Target_val).toFixed(2));
                $('#paramid_concfeed_target').text((value.KPI_ConcFeed_Target_val).toFixed(2));
                $('#paramid_culoss_target').text((value.KPI_CuLossInSlag_Target_val).toFixed(2));
                $('#paramid_bin_target').text();
                $('#paramid_slagtemp_target').text((value.KPI_SlagTemp_Target_val).toFixed(2));
                $('#paramid_slaglvl_target').text((value.KPI_SlagLevel_Target_val).toFixed(2));
                $('#paramid_bathlvlh1_target').text((value.KPI_BathLevelH1_Target_val).toFixed(2));
                $('#paramid_drafta_target').text((value.KPI_FSFDraftA_Target_val).toFixed(2));
                $('#paramid_draftb_target').text((value.KPI_FSFDraftB_Target_val).toFixed(2));
                $('#paramid_whbtmp_target').text((value.KPI_WHBInletTemp_Target_val).toFixed(2));
                $('#paramid_cyccompl_target').text((value.CyclesCompleted_Target_val).toFixed(2));
                $('#paramid_blistprod_target').text((value.BlisterProduction_Target_val).toFixed(2));
                $('#paramid_ttlsecond_target').text((value.CFSecondariesCharged_Target_val).toFixed(2));
                $('#paramid_blowtime_target').text((value.KPI_CFBlowingTime_Target_val).toFixed(2));
                $('#paramid_blisttrd_target').text((value.KPI_AFBlisterTreated_Target_val).toFixed(2));
                $('#paramid_lotscasted_target').text((value.LotsCompleted_Target_val).toFixed(2));
                $('#paramid_anodeprod_target').text((value.AnodeCasted_Target_val).toFixed(2));
            });
