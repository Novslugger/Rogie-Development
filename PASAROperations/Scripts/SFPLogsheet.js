    ...
    $.ajax({
        ...
        success: function (data) {
            $.each(data, function (index, value) {
                ...
                $('#sfp_tbl tr:last').after('<tr><td id="locationname">' + value.SFP_PrevLocationName + '</td><td id="initlocationname">' + (value.SFP_LocationName ? value.SFP_LocationName : "-") + '</td><td id="date_delivered">' + formatDate_af(value.SFP_DTime) + '</td><td id="potno">' + value.SFP_PotId + '</td><td id="ctime">' + value.SFP_CoolTime + '</td><td id="expdate_' + value.SFP_DataId + '">' + formatDate_af(value.SFP_ExpectedDTime) + '</td><td id="actualtime">' + formatDate_af(value.SFP_ActualDTime) + '</td><td id="dumptemp">' + (value.SFP_DumpTemp ? value.SFP_DumpTemp : "-") + '</td><td id="sfpremark">' + (value.SFP_Remark ? value.SFP_Remark : "-") + '</td><td><a href="#" onclick="load_sfp_edit(' + value.SFP_DataId + '); return false;">&nbsp;&nbsp;&nbsp;<u>Edit</u>&nbsp;&nbsp;&nbsp;</a></td><td><a href="#" onclick="load_sfp_delete(' + value.SFP_DataId + '); return false;">&nbsp;&nbsp;&nbsp;<u>Delete</u>&nbsp;&nbsp;&nbsp;</a></td><tr>');
...
function load_sfp_complete_data(furnace_) {
    $.ajax({
        ...
        success: function (data) {
            $.each(data, function (index, value) {
                $('#page_id').text(value.SFP_Val_GroupId);
                $('#sfp_tbl tr:last').after('<tr><td id="locationname">' + value.SFP_PrevLocationName + '</td><td id="initlocationname">' + (value.SFP_LocationName ? value.SFP_LocationName : "-") + '</td><td id="date_delivered">' + formatDate_af(value.SFP_DTime) + '</td><td id="potno">' + value.SFP_PotId + '</td><td id="ctime">' + value.SFP_CoolTime + '</td><td id="act_ctime">' + subHours(formatDate_af(value.SFP_ActualDTime), formatDate_af(value.SFP_DTime)) + '</td><td id="expdate">' + formatDate_af(value.SFP_ExpectedDTime) + '</td><td id="actualtime">' + formatDate_af(value.SFP_ActualDTime) + '</td><td id="dumptemp">' + (value.SFP_DumpTemp ? value.SFP_DumpTemp : "-") + '</td><td id="sfpremark">' + (value.SFP_Remark ? value.SFP_Remark : "-") + '</td><td><a href="#" onclick="load_sfp_edit_c(' + value.SFP_DataId + '); return false;">&nbsp;&nbsp;&nbsp;<u>Edit</u>&nbsp;&nbsp;&nbsp;</a></td><td><a href="#" onclick="load_sfp_delete_c(' + value.SFP_DataId + '); return false;">&nbsp;&nbsp;&nbsp;<u>Delete</u>&nbsp;&nbsp;&nbsp;</a></td></tr>');
            });
            if (data.length > 0)
                paginate('sfp_tbl', 21);
        }
...
$('#group_ddl').live('change', function () {
    if ($(this).val() == 1) {
        $('#cooltime_input').val('60');
        $('#cooltime_input2').val('60');
    } else {
        $('#cooltime_input').val('30');
        $('#cooltime_input2').val('30');
    }
    calc_edtime_add_cf();
});
        ...
        success: function (data) {
            ...
            $.each(data, function (index, value) {
                if (value.DataId != null && value.SFP_Location_StatusId == 1) {
                    ...
                    var isOverdue = check_data_overdue(value.SFP_ExpectedDTime);
                ...
                if (value.SFP_Location_StatusId == 1) {
                    $('.status_' + value.SFP_Location_StatusId).addClass('active');
                }

                if (isOverdue) {
                    $('td[id="paramname_' + value.LocationId + '"]').append('<span class="overdue" style="margin-right: 5px; margin-left: 5px;">');
                }
        ...
        success: function (data) {
            $.each(data, function (index, value) {
                if (value.SFP_ParamId == 256 || value.SFP_ParamId == 257)
                    ...
                    $('#paramid_' + value.SFP_ParamId + ' input').val(value.SFP_Comment_Val);
            });
        }
    });
}
